...
//ENVIRONMENT SETUP
float VEGFconc; //for uniform VEGF above a vessel JTB 2008
...
//Samra APREDDICT study 
float DrugSupplied_tumour_nM = 0;
...
float accumulatedDrug_store_Gut[MAXtime];
float accumulatedDrug_store_Blood_mgL[MAXtime];
float accumulatedDrug_store_Blood_nM[MAXtime];
float supplyLine_store[MAXtime];
float drugEffect_VR2_store[MAXtime];
float Dll4_store[MAXtime];
...
void readArgs(int argc, char * argv[]) {
    //you can ignore this for APREDICT study it is not used.  
    if (argc > 1)
...
/*
    ...
    */
     ...
     //APREDICT study read in build args order:
     
     drug_delivery_schedule =  atoi(argv[1]); //2880 12 hrs
     dailyDose = atof(argv[2]);

    readInGradient=atoi(argv[3]);
   
     VconcST = atof(argv[4]);
     VEGFconc = atof(argv[5]);

     RUNS = atoi(argv[6]);
    
     //seed = stoll(argv[11]);

     //cout<<drug_delivery_schedule<<" "<<dailyDose<<" "<<VconcST<<" "<<supplyScalar<<" "<<RUNS<<endl;


    char outfilename[500];
    sprintf(outfilename, "%d_dose_%g_gradient_%d_VconcST_%g_Vconc_%g_run_%d.csv",drug_delivery_schedule,dailyDose,readInGradient,VconcST,VEGFconc,RUNS);
    //char outfilename2[500];
    //sprintf(outfilename, "analysis_APREDICT_Blood_doseschedule_%g_dose_%g_Vconc_%g_Protein_binding_percent_%g_run_%g",drug_delivery_schedule,dailyDose,VEGFconc,RUNS);
   ...
   //RUNSfile2.open(outfilename2);
    ...
    WORLDpointer = world;
    world->VEGFgradient = readInGradient;
    ...
    while (timeStep <= MAXtime)
    {
       //if (timeStep % 50 == 0) cout << "timestep " << timeStep << ".. " << MAXtime - timeStep << " left" << endl;
        ...
        if (timeStep == MAXtime){
            ...
            DrugSupplied_store_Tumour[0]=0.0f;
        accumulatedDrug_store_Gut[0]=0.0f;
        accumulatedDrug_store_Blood_mgL[0]=0.0f;
        accumulatedDrug_store_Blood_nM[0]=0.0f;
        supplyLine_store[0]=0.0f;
        drugEffect_VR2_store[0]=0.0f;
        Dll4_store[0]= 0.0f;

            RUNSfile<<drug_delivery_schedule<<"\t"<<dailyDose<<"\t"<<readInGradient<<"\t"<<VconcST<<"\t"<<VEGFconc<<"\t"<<Protein_binding_percent<<"\t"<<RUNS<<endl<<endl;
            for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<((float)i*15.0)/60.0/60.0<<"\t";
            }
            } 
            RUNSfile << endl;
            ...
            for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<DrugSupplied_store_Tumour[i]<<"\t";
            }
            } 
            RUNSfile << endl;
                ...
                for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<accumulatedDrug_store_Gut[i]<<"\t";
            } 
        }
        RUNSfile << endl;
        ...
        for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<accumulatedDrug_store_Blood_mgL[i]<<"\t";
            } 
        }
            RUNSfile << endl;
         ...
         for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<accumulatedDrug_store_Blood_nM[i]<<"\t";
            } 
        }
            RUNSfile << endl;
...
if(i%60==0){
        ...
        for(i=0;i<MAXtime;i++){
if(i%60==0){
                RUNSfile<<drugEffect_VR2_store[i]<<"\t";
            } 
        }
            RUNSfile << endl;
...
void World::simulateTimestep()
{
    ...
    {
        if(ANALYSIS_APREDICT){
            //output_cell_protlevels(RUNSfile);
            
            //DRUG in the GUT---------------
            //daily dose and decay in gut from oral drug, this is the small amount transferring over to the blood stream each timestep.. (Tmax  = 2.5hrs)        
            accumulated_DrugDose_Gut -= DrugAbsorption*0.693*accumulated_DrugDose_Gut; 

            //give drug dose every X timesteps based on schedule
            if(timeStep%drug_delivery_schedule==0)accumulated_DrugDose_Gut += dailyDose;

            //one off dose
            //if(timeStep==drug_delivery_schedule)accumulated_DrugDose_Gut += dailyDose;
            
            //DRUG in the BLOOD---------------
            
            //drug loss in blood
            //accumulated_DrugDose_Blood_mgL-= DrugHalfLife*0.5*accumulated_DrugDose_Blood_mgL;
            
            //0.693 is an approx of natural log (better than 0.5). (1/halflife)*timestepsize*ln(2) (drughalflife is already incorporating the timstepsize)
            accumulated_DrugDose_Blood_mgL-= DrugHalfLife*0.693*accumulated_DrugDose_Blood_mgL;

            if(accumulated_DrugDose_Blood_mgL<0) accumulated_DrugDose_Blood_mgL =0;

            //drug accumulating in blood, scaled absorption from gut, we know some lost on the way. (VD_F value)
            accumulated_DrugDose_Blood_mgL +=(DrugAbsorption*0.693*accumulated_DrugDose_Gut)/VD_F;

            //Converting: mg/L * 1e-3 g/mg * (1/389.469) mol/g * 1e+9 nmol/mol => nmol/L = nMso the 0.02 is divided by 389.469 (the molecular weight) and multiplied by 10^6 = 51.35 nM, well above the IC50
            accumulated_DrugDose_Blood_nM = (accumulated_DrugDose_Blood_mgL/389.469)*1000000;
            

            //Drug in Tumour------------(scaled by vascular supplylines quality)

            supplyLine_Score = APREDICT_Supply_lines_Score();
            //cout<<"average supplyline score over time window: "<<supplyLine_Score<<endl;
           
            //quality of brnaching scales to a 0-1 sclare 1= perfect normalised brnaching, 0 = no new branches
            //vascularSupplied = supplyScalar*supplyLine_Score;
            //if(vascularSupplied>1) vascularSupplied = 1;

            DrugSupplied_tumour_nM = Protein_binding_percent*(baseSupply*accumulated_DrugDose_Blood_nM+((1-baseSupply)*accumulated_DrugDose_Blood_nM*supplyLine_Score));
            //if(DrugSupplied>1) DrugSupplied=1;

            //for analysis-------------
            accumulatedDrug_store_Gut[timeStep]=accumulated_DrugDose_Gut;
            accumulatedDrug_store_Blood_mgL[timeStep]=accumulated_DrugDose_Blood_mgL;
            accumulatedDrug_store_Blood_nM[timeStep]=accumulated_DrugDose_Blood_nM;
            supplyLine_store[timeStep]= supplyLine_Score;
            DrugSupplied_store_Tumour[timeStep]=DrugSupplied_tumour_nM;
            drugEffect_VR2_store[timeStep] = drugEffect_VR2;

            //sum up dll4 levels over cells to see if oscillating or inhibited. 
            dll4sum=0;
            for(i=0;i<ECELLS;i++){

                dll4sum += ECagents[i]->Dll4tot;
            }
            Dll4_store[timeStep] = dll4sum/ECELLS;
            //-----------------------------

             if(timeStep%100==0){
               cout<<"timestep: "<<timeStep<<" accumDrug_Gut: "<<accumulated_DrugDose_Gut<<" accumDrug_Blood mgL"<<accumulated_DrugDose_Blood_mgL<<" accumDrug_Blood_nM"<<accumulated_DrugDose_Blood_nM<<" supplyLine_Score: "<<supplyLine_Score<<" drugEffect_VR2: "<<drugEffect_VR2<<" dll4ave: "<<dll4sum/ECELLS<<endl;
               // cout<<"average supplyline score over time window: "<<supplyLine_Score<<endl;
                
              // RUNSfile<<accumulated_DrugDose_Gut<<"\t";
              // RUNSfile2<<accumulated_DrugDose_Blood<<"\t";

               //RUNSfile<< supplyLine_Score<<"\t";
            }

            //cout<<"accumulated drug: "<<accumulated_DrugDose<<"supplied: "<<DrugSupplied<<endl;

        }

        //SAMRA APPREDICT 
       // accumulated_DrugDose += 1.0f; //take a pill every day 

        //slidingWindowVascularProfile +=        
        //DrugOccupancy = (accumulated_DrugDose*DrugHalfLife)*slidingWindowVascularProfile;
...
// comment out to test drug dynamics no cell sims.. -------
        updateMemAgents();
...
//cout<<endl;
    ...
    for (j = 0; j < (int) ECagents.size(); j++) {
        ...
        /*cout<<"cell: "<<j<<"stableArray: ";

        for(k=0;k<10;k++){

            if(ECagents[j]->stableArray.size()-k-1>=0)
            cout<<ECagents[j]->stableArray[ECagents[j]->stableArray.size()-k-1]<<" ";
        }
        cout<<endl;*/
