...
//---------------------------------------------------------------------------------------------
float World::APREDICT_Supply_lines_Score(void)
{
    int non_adjacent_Tips=0;
    float supplyScore=0.0f;
    ...
    EC* ecpNeighL;
    EC* ecpNeighR;
    int nonAdj_tips[ECELLS];
    float aveStability_nonadj_tips=0.0f;

    float aveSuppScore = 0.0f;
    float avetipstability =  0.0f;
    ...
    for (c = 0; c < ECELLS; c++)
    {
        ...
        //if its a tip cell  check if its adjacent to another tip or not. 
        if (ecp->tipCellTest())
        {
            tipCount++;

            //look to the left and right neighs to check if its a tip (with periodic boundariy conditions)
            if((c!=0)&&(c<ECELLS-1)){
                
                ecpNeighL = ECagents[c-1];
                ecpNeighR = ECagents[c+1];
            }
            else if(c==0){

                ecpNeighL = ECagents[ECELLS-1];
                ecpNeighR = ECagents[c+1];

            }
            else if(c==ECELLS-1){
                ecpNeighL = ECagents[c-1];
                ecpNeighR = ECagents[0];
            }
            
            if(ecpNeighL->tipCellTest()){
                adjacent_Tips++;
            }
            else if(ecpNeighR->tipCellTest()){
                adjacent_Tips++;
            }
            else{
                non_adjacent_Tips++;
                nonAdj_tips[c]=1;
                if(ecp->stableArray.size()<APREDICT_slidingWindow){

                    //vector<int>::iterator iter = ecp->stableArray.end();
                   // cout<<"less: stable array cell: "<<c;
                     ...
                     do{ 
                        ...
                        i++;
                        avetipstability++;
                       // cout<<" "<<ecp->stableArray[ecp->stableArray.size()-i-1];

                     }while((ecp->stableArray[ecp->stableArray.size()-i-1]==1)&&(i<ecp->stableArray.size()));
                        
                    
                    //cout<<" ave: "<<avetipstability<<endl;
                }
                else{
                     //vector<int>::iterator iter = ecp->stableArray.end();
                    // cout<<"more: stable array cell: "<<c;
                     i=-1;
                     ...
                     }while((ecp->stableArray[ecp->stableArray.size()-i-1]==1)&&(i<APREDICT_slidingWindow));
...
if(non_adjacent_Tips>0){
    //cout<<" ave: "<<(float)avetipstability/(float)non_adjacent_Tips<<endl;
    
     aveStability_nonadj_tips = ((float)avetipstability/(float)non_adjacent_Tips)/(float)APREDICT_slidingWindow;
     if(aveStability_nonadj_tips>1) aveStability_nonadj_tips=1.0f;
   // cout<<"non_adjacent_Tips: "<<non_adjacent_Tips<<" aveStability_nonadj_tips: "<<aveStability_nonadj_tips<<endl;
...
sliding_Window_supplyScore.push_back(supplyScore);

sliding_Window_supplyScore.erase(sliding_Window_supplyScore.begin());


for(i=0;i<sliding_Window_supplyScore.size();i++){
    //cout<<" "<<i<<": "<<sliding_Window_supplyScore[i];
    aveSuppScore +=sliding_Window_supplyScore[i];
    
   

}

//cout<<"ave_supscore: "<<(float)aveSuppScore/(float)APREDICT_slidingWindow<<endl;

return(((float)aveSuppScore/(float)APREDICT_slidingWindow) * aveStability_nonadj_tips);
//return(aveSuppScore/(float)APREDICT_slidingWindow);

//return((aveSuppScore/(float)APREDICT_slidingWindow)*(avetipstability/non_adjacent_Tips));
//cout<<endl;
